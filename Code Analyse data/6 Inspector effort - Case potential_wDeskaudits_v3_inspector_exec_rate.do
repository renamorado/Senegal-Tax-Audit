...
local dvlabel_exec_rate      "Outcome: Execution Rate of Inspector-Selected Cases"
...
foreach dv of local dvlist {
    ...
    * ---------------- Export ----------------
    if (`audtype' == 1) {
        ...
        esttab `models' using "$output/inspectors_reg_discrete_`audit_type'_`dv'_inspector.tex", replace ///
            booktabs ///
            keep(1.good 1.bad) ///
...
foreach dv of local dvlist {
    ...
    if (`audtype' == 1) {
        ...
        esttab `models' using "$output/inspectors_reg_share_`audit_type'_`dv'_inspector.tex", replace ///
            booktabs ///
            keep(share_good) ///
...
preserve
	...
	graph export "$output\execution_rate_boxplot_`audit_type'_inspector.pdf", replace
restore
...
********************************************************************************
local outpan "$output/inspectors_reg_panels_Full_exec_rate_inspector.tex"
local dvlabel_exec_rate "Outcome: Execution Rate of Inspector-Selected Cases"
...
local headA "\multicolumn{`span'}{l}{\textbf{Dependent variable}: Outcome: Execution Rate of Inspector-Selected Cases}\\ \addlinespace \multicolumn{`span'}{l}{\textbf{Panel A: Discrete measures}}\\ \addlinespace \midrule"

* --- Build the custom header row (DV in stub + the 9 column titles) ---
local dvrow "\multicolumn{1}{l}{\textbf{Outcome: Execution Rate of Inspector-Selected Cases}} & Top 2 & First 10\% seq & First 20\% seq & Top 2 & First 10\% seq & First 20\% seq & Top 2 & First 10\% seq & First 20\% seq \\"
