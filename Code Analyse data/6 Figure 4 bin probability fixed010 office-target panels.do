...
*
* This robustness version defines the realized top-half evasion target within
* audit type x detailed tax office, pooling across selection years. The x-axis
* uses fixed-width bins of the predicted probability that an audit case belongs
* to the top half of realized evasion under that office-pooled target.
...
local binprob_input "$output\figure4_binprob_decile_fullaudits_c1_office.dta"
local bin_width 0.10
local bin_count 10
local bin_tag "fixed010"
local suffix "office_target"
local xtitle_text "Predicted probability of above-median evasion"

if "$figure4_binprob_input" != "" {
    local binprob_input "$figure4_binprob_input"
}
if "$figure4_bin_width" != "" {
    local bin_width = real("$figure4_bin_width")
}
if "$figure4_bin_count" != "" {
    local bin_count = real("$figure4_bin_count")
}
if "$figure4_bin_tag" != "" {
    local bin_tag "$figure4_bin_tag"
}
if "$figure4_suffix" != "" {
    local suffix "$figure4_suffix"
}
if "$figure4_xtitle" != "" {
    local xtitle_text "$figure4_xtitle"
}
...
label var binprob_fixed_bin "Fixed-width probability bin"
...
local support_output "$output\figure4_binprob_`bin_tag'_fullaudits_`suffix'_support.tex"
...
graph export "$output\figure4_panel_a_binprob_`bin_tag'_fullaudits_`suffix'.pdf", replace
...
graph export "$output\figure4_panel_b_binprob_`bin_tag'_fullaudits_`suffix'.pdf", replace
