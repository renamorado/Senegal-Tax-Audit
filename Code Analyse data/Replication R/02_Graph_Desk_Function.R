...
prob_execution <- fullaudits_predicted %>%
  filter(yhatrf > 10) %>%
  mutate(
    # Create quintiles based on yhatrf using quantiles (0%, 20%, 40%, 60%, 80%, 100%)
    bin = cut(
      yhatrf,
      breaks = quantile(yhatrf, probs = seq(0, 1, by = 0.2), na.rm = TRUE),
  ...
  summarise(
    # Mean of y2 in this bin
    prob = mean(y2, na.rm = TRUE),
    # Sample variance of y2 in this bin
...
#predicted_percent <- predicted_percent %>% filter(mid_bin > 10)
