...
egen bureauselectionyearmethod = group(bureau_detailed selectionyear alg_rand)

egen quartiles_er= xtile(evasionrate), by(bureauselectionyearmethod)  nq(4)   // or nquantiles(4)
...
forval sample = 1/3 {
	...
	*local sample = 1 
		...
		*
		collapse (sum) total_assigned total_executed null_evasion belowmedian_er bottomquartile_er  `cond`sample'', by(method bureau_detailed verificateur_selection selectionyear) 
		
		
		*Collapsing at the inspector - bureau - selectionyear level
		*collapse (sum) total_assigned total_executed null_evasion `cond`sample'', ///
		by(method groupbureau verificateur_selection selectionyear) 
		...
		*reshaping to obtain the audit numbers (method as columns)
		...
		i(verificateur_selection bureau_detailed selectionyear) j(method, string)
		...
		*tagging inspectors that changed from bureau
		egen tag_verificateur = tag(verificateur_selection)
		

		** Tagging inspectors that are observed in the same bureau 
		egen tag_verifbureau = tag(verificateur_selection bureau_detailed)
		bys verificateur_selection bureau_detailed: gen verif_bureau_consecutive = _n

		*check if verificateur didn't changed from office

		bys verificateur_selection: egen sum_tags = total(tag_verifbureau) // 
	...
	* Ensure proper sort
	sort verificateur_selection selectionyear bureau_detailed

	* Mark first observation of each inspector-year
	by verificateur_selection selectionyear: gen first_in_year = _n == 1

	* Cumulative sum of "first_in_year" within inspector = year sequence
	by verificateur_selection: gen year_seq = sum(first_in_year)
	
	*tag inspectors by the max periods they wer observed
	bys verificateur_selection: egen max_period = max(year_seq)
...
* collapse at the inspector year level		
	collapse (sum) total_assigned* total_executed* null_evasion* bottomquartile_er* belowmedian_er*, by(verificateur_selection selectionyear unchanged)
	egen tag_verificateur = tag(verificateur_selection)
		** Generating total
		*Total execution
...
*generating shares among methods
		...
		*make the scatter plot 
		...
		keep verificateur_selection selectionyear  unchanged_bureau share_v_ALG_all share_v_ALG ///  
...
* sample condition (avoid missing(x,y,z) with commas to keep parsing robust)
* sample condition
* sample condition
...
* Build legend order/labels dynamically (ONLY include fit labels if fit exists)
local legorder "1"
