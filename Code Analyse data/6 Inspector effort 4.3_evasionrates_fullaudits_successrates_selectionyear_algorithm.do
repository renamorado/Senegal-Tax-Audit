...
twoway (kdensity evasionrate if selectionyear==2018) || ///
    ...
    pos(6) ring(1) cols(3))
graph export "$output/kdensity_evasionrate_syear_fullaudits_success_selectionyear_algorithm.pdf", replace

	
**# 3. tag cases with above median and top quartile evasionrate within year-algorithm
egen taglist= tag(selectionyear algorithm) 

*egen group= group(selectionyear algorithm)
count if taglist  ==1 // unique year-algorithm groups



*ssc install egenmore, replace
egen quartiles_er= xtile(evasionrate), by(selectionyear algorithm)  nq(4)   // or nquantiles(4)
...
**# Checking 
* How many unique year-algorithm groups?
preserve
keep selectionyear algorithm
duplicates drop
tab selectionyear algorithm
restore

* Quartiles should vary within each year-algorithm group
by selectionyear algorithm, sort: tab quartiles_er, missing
...
twoway `plotcmd', ///
	...
	graph export "$output/scatter_`share'_`t'_`t1'_full_success_selectionyear_algorithm.pdf", replace
	}
...
foreach relshare in rel_share_exec_Algorithm rel_share_det_Algorithm ///
	...
	graph export "$output/scatter_`relstub'_`t'_`t1'_full_success_selectionyear_algorithm.pdf", replace
}
...
esttab bureau_stats_1 ///
       bureau_stats_2 ///
       bureau_stats_3 ///
       using "$output\bureau_sample_stats_success_selectionyear_algorithm.tex", replace ///
...
esttab bureau_has_1 ///
       bureau_has_2 ///
       bureau_has_3 ///
       using "$output\bureau_has_exec_detected_t_t1_success_selectionyear_algorithm.tex", replace ///
    main(sum) noobs nonote ///
    order( ///
        has_executed_all_t has_executed_Algorithm_t has_executed_Inspectors_t ///
        has_detected_all_t     has_detected_Algorithm_t     has_detected_Inspectors_t ///
...
esttab bureau_share_1 ///
       bureau_share_2 ///
       bureau_share_3 ///
       using "$output\bureau_share_exec_success_t_t1_success_selectionyear_algorithm.tex", replace ///
    noobs nonote  ///
    main(mean ) ///
    aux(sd) ///
    order( ///
