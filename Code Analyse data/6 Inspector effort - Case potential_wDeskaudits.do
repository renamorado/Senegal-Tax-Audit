...
foreach audtype in 1 0 {
	...
	* from 07_Optimization_Excercise.R
	...
	/*
	*from 06_Compare_Algorithm_vs_inspector
...
************************************************************
	...
	* Quintiles of yhatrf WITHIN list (among all methods cases)
	if `audtype'==0 {
		...
		egen yhatrf_q5 = xtile(yhatrf), by(bureauselectionyear algorithm)  nq(5)
		egen yhatrf_q4 = xtile(yhatrf), by(bureauselectionyear algorithm)  nq(4) 
	}
	else {
		egen yhatrf_q5 = xtile(yhatrf), by(inspectorclusteryear)  nq(5)   // or nquantiles(4)
		egen yhatrf_q4 = xtile(yhatrf), by(inspectorclusteryear)  nq(4)   // or nquantiles(4)
	}
