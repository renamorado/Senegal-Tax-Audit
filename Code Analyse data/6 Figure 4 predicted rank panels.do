...
* the unweighted Random Forest prediction of full-audit list rank. The x-axis
* uses the inverted predicted rank score, so higher values correspond to higher
* predicted evasion priority.
...
local rank_input "$analysisdata\fullaudits_predicted_rank_bureauyear.dta"

************************************************************
* 1. Load selected non-safety full audits
************************************************************
use "`rank_input'", clear

foreach var in yhatrf_rank_score predicted_rank_y4_bureauyear predicted_rank_order_bureauyear rank_y4_bureauyear y2 y4 algorithm dgid selection safeties {
    capture confirm variable `var'
    if _rc {
        di as error "Required variable `var' not found in `rank_input'."
        exit 111
    }
}
...
drop if missing(yhatrf_rank_score)
...
keep if !missing(y2)
gen mid_bin = floor(yhatrf_rank_score) + 0.5
...
use `fullaudits', clear
keep if y2 == 1 & !missing(rank_y4_bureauyear)
gen mid_bin = floor(-rank_y4_bureauyear) + 0.5
collapse (count) n_realized = rank_y4_bureauyear, by(mid_bin)
egen total_realized = total(n_realized)
gen realized_rank_percent = 100 * n_realized / total_realized
keep mid_bin realized_rank_percent
...
foreach var in predicted_nonexecuted_percent predicted_executed_percent realized_rank_percent {
...
twoway ///
    ...
    (area realized_rank_percent mid_bin, fcolor(gs8%15) lcolor(gs8%0) lwidth(vvthin)) || ///
    ...
    ytitle("Percentage (%)") ///
    xtitle("Inverted predicted rank score") ///
    ylabel(0(25)100, grid) ///
    xlabel(, grid) ///
    legend(order(2 "Predicted rank, executed" ///
                 1 "Predicted rank, non-executed" ///
                 3 "Realized rank score" ///
                 5 "P(execution|inverted predicted rank)") ///
           rows(2) position(6) region(lcolor(none))) ///
    graphregion(color(white)) ///
    plotregion(color(white))

graph export "$output\figure4_panel_a_predicted_rank_fullaudits.pdf", replace
...
twoway ///
    ...
    legend(order(1 "Algorithm Cases Distribution" ///
                 ...
                 5 "Algorithm Cases P(execution|inverted predicted rank)" ///
                 6 "Inspector Cases P(execution|inverted predicted rank)") ///
           rows(2) size(small) position(6) region(lcolor(none))) ///
    graphregion(color(white)) ///
    plotregion(color(white))

graph export "$output\figure4_panel_b_predicted_rank_fullaudits.pdf", replace
