				...
				* Bins are computed either within selection method or on the pooled sample.
				if "`bin_def'" == "decile" {
					...
					if "`b'" == "yhatrf" {
						...
						else local xti_bin "Whole-sample decile of Predicted evasion (log FCFA)"
					}
					else if "`b'" == "turnover_mean" {
						if "`bin_scope'" == "within_method" local xti_bin "Decile of Firm Size (Mean turnover 2014-2020)"
						else local xti_bin "Whole-sample decile of Firm Size (Mean turnover 2014-2020)"
					}
				}
					...
					if "`b'" == "yhatrf" {
						...
						else local xti_bin "Whole-sample quintile of Predicted evasion (log FCFA)"
					}
					else if "`b'" == "turnover_mean" {
						if "`bin_scope'" == "within_method" local xti_bin "Quintile of Firm Size (Mean turnover 2014-2020)"
						else local xti_bin "Whole-sample quintile of Firm Size (Mean turnover 2014-2020)"
					}
				}
				else {
					* --- FIXED-WIDTH BINS ---
					...
					if "`bin_scope'" == "within_method" {
						...
						egen min_raw_`b' = min(`b')
						egen max_raw_`b' = max(`b')
					}
					...
					if "`b'" == "yhatrf" {
						...
						else local xti_bin "Whole-sample bins of Predicted evasion (log FCFA), width = `w'"
					}
					else if "`b'" == "turnover_mean" {
